...
class Regexp extends FunctionNode
{
    ...
    public function parse(\Doctrine\ORM\Query\Parser $parser)
    {
        ...
        $parser->match(Lexer::T_COMMA);
        $this->caseSensitivity = $parser->StringExpression();
    ...
    {
        if (strtolower($this->caseSensitivity->dispatch($sqlWalker)) == "'sensitive'") {
            // collate to case sensitive charset, and use REGEX BINARY
            return '(' . $this->value->dispatch($sqlWalker) . ' COLLATE utf8_bin  REGEXP BINARY ' . $this->regexp->dispatch($sqlWalker) . ')';
        } else {
